...
| ファイル | 扱い | 備考 |
|---|---|---|
| `platza/lib/firebase_options.dart` | **コミット可** | Emulator 用 demo 設定。実プロジェクト切替前のみ有効 |
| `platza/lib/firebase_options_dev.dart` | **gitignore** | `flutterfire configure` で生成。次ストーリで導入 |
| `platza/lib/firebase_options_prod.dart` | **gitignore** | 同上 |
| `ios/Runner/GoogleService-Info.plist` | **gitignore** | Firebase Console からダウンロード、ローカル & CI Secret で管理 |
| `android/app/google-services.json` | **gitignore** | 同上 |
| `firestore.rules` / `storage.rules` | **コミット** | レビュー対象。`firebase deploy --only firestore:rules` で反映 |
| `firebase.json` / `.firebaserc` | **コミット** | `.firebaserc` の alias 設定は次ストーリで追加 |
| API キー類（FCM サーバキー等） | **gitignore + GitHub Secrets** | CI から参照する場合のみ Secrets に登録 |

**原則**: `flutterfire configure` で生成される flavor 別の `firebase_options_*.dart`
とネイティブ設定ファイル (`GoogleService-Info.plist` / `google-services.json`) は
リポジトリに含めない。配布が必要な開発者には別チャネルで共有する。

CI では GitHub Secrets から復元してビルド時に配置する運用を想定（次ストーリで具体化）。
