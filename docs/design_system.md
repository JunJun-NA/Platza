...
| CareLogEntryRow | `emoji`, `title`, `time`, `note` | title/emoji は careType から derive されるが Figma では override 可能 |
| CareEventCard | `emoji`, `title`, `statusLabel`, `time`, `note` | statusLabel は state variant と連動しない（Property override） |
...
**必須**: 新規 Figma コンポーネント作成時は、**コンポーネント内に表示される動的テキストを全て TEXT Component Property として公開する**。ハードコードのまま残すのは NG（デザイナーがインスタンスで差し替えできなくなる）。

**Property 化する対象** — Flutter 側で値が動的に変わるテキスト：

- widget コンストラクタの `String` / `String?` パラメータ（`title`, `subtitle`, `time`, `note` など）
- enum / bool から derive されるラベル（`careType.label` → `title`、`careType.emoji` → `emoji`、`isPerformed ? '実施済み' : '予定'` → `statusLabel` 等）
- variant 固有の Property（例: tappable の `hintLabel`）は該当 variant のテキストノードのみにバインド

**運用**:

- Property 名は Flutter 側の意味と合致させる（widget パラメータ名そのまま、または derive の役割名）
- コンポーネント作成直後に `componentPropertyDefinitions` を確認し、内部の動的テキストと漏れなく対応しているかセルフチェック
- variant property とテキスト property を併用するときの挙動: variant 切替でテキストは自動切替されない（Figma の仕様）。Property 側は override と位置付ける
